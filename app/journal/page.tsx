...
function JournalPageContent() {
    ...
    const viewingPost = viewingPostId ? journalPosts.find(p => p.id === viewingPostId) : null;
                    ...
                    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
                        <div className="w-full max-w-4xl max-h-[90vh] bg-[#FFFEF9] rounded-[32px] shadow-2xl flex flex-col overflow-hidden relative">
                            ...
                            <div className="flex-none flex items-center justify-between px-8 py-6 md:px-12 md:py-8 border-b border-[#F5E6D3] bg-[#FFFEF9]">
                                <div className="flex items-center gap-3">
                                    <span className={cn(
                                        ...
                                        {categories.find(c => c.id === viewingPost.category)?.label || '기록'}
                                    </span>
                                    {viewingPost.material_status && (
                                        <span className={cn(
                                            "px-3 py-1 rounded-full text-[10px] font-bold uppercase",
                                            viewingPost.material_status === 'draft' ? "bg-amber-100 text-amber-600" : "bg-green-100 text-green-600"
                                        )}>
                            ...
                            <div className="flex-1 overflow-y-auto p-8 md:p-12 bg-[#FFFEF9]">
                                <h2 className="text-3xl font-black text-[#37352F] mb-6 tracking-tight">{viewingPost.title}</h2>
                                ...
                                <div className="flex flex-wrap gap-4 pt-8 border-t border-[#F5E6D3] mb-12">
                                    <div className="flex items-center gap-2 px-4 py-2 bg-white border border-[#F5E6D3] rounded-xl text-xs font-bold text-[#37352F]">
                                        <User size={14} className="text-[#A1A1A1]" /> {getUserName(viewingPost.user_id)}
                                    </div>
                                    <div className="flex items-center gap-2 px-4 py-2 bg-white border border-[#F5E6D3] rounded-xl text-xs font-bold text-[#A1A1A1]">
                                        {new Date(viewingPost.created_at).toLocaleString('ko-KR')}
                                    </div>
                                    {viewingPost.book_id && (
                                        <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-100 rounded-xl text-xs font-bold text-blue-600">
                                            <BookOpen size={14} /> {getBookTitle(viewingPost.book_id)}
                                        </div>
                                    )}
                                </div>

                                {/* Links & Files */}
                                {((viewingPost.links && viewingPost.links.length > 0) || (viewingPost.files && viewingPost.files.length > 0)) && (
                                ...
                                <div className="bg-[#FFFCF5] rounded-[32px] border border-[#F5E6D3] p-8 md:p-10 shadow-sm">
                                    <div className="flex items-center gap-3 mb-8">
                                        ...
                                        <h3 className="text-xl font-black text-[#37352F] tracking-tight">댓글 {viewingPost.comments?.length || 0}</h3>
                                    </div>
                                            ...
                                            <div key={comment.id} className="bg-white p-5 rounded-2xl border border-[#F5E6D3] shadow-sm flex flex-col gap-2">
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        {(currentUser?.id === comment.user_id || currentUser?.id === viewingPost.user_id) && (
                                                            <button onClick={() => deleteComment(viewingPost.id, comment.id)} className="text-[#A1A1A1] hover:text-rose-500 transition-colors">
                                                                <X size={14} />
                                                            </button>
                                                        )}
                                                    </div>
